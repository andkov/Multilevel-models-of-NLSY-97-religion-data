...
colorGroupAlpha <- adjustcolor(colorGroup, alpha.f=.02) #Define translucent colors for each group
...
# cohort<-1981
# ds<-dsattendlong [which(dsattendlong$byear==cohort & dsattendlong$id>1),]
# ds<-dsattendlong [which(dsattendlong$byear==cohort),]
# ds$byearci<-ds$byearc+1 # 
...
for( i in unique(ds$id) ) { 
  ...
  xs <- jitter(dsSlice$time,1) #Should we jitter?
  ys <- dsSlice$attend
  #lines(x=xs, y=ys)
...
b<-.1
y<-a+b*x
plot.window(xlim=c(2000 2010), ylim=c(1 8))
curve(a+b*x, from=0, to=10,add=F)
...
curve(co["(Intercept)"]  + co["timec"]*x , add=T, col="red", lwd=4)
